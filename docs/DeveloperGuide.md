...
1. **Initial launch**
   ...
   **Expected:**
   The GUI appears with a set of sample contacts. The window size may not be at its optimum on first launch.
...
2. **Saving window preferences**
    ...
    1. Resize the window to a preferred size and move it to a different location.
    2. Close the application.
    3. Re-launch the app by double-clicking the JAR file.

   **Expected:**
   The most recent window size and location are retained.

3. **Closing the application normally**

    1. Launch the application.
    2. Close the window using the window close button.
...
1. **Listing all athletes when the address book is empty**

    1. **Prerequisites:** The file `data/addressbook.json` contains an empty list.
    2. **Test case:** `list`

   **Expected:**
   No athletes are shown in the athlete list panel.

2. **Listing all athletes when the address book contains multiple athletes**

    1. **Prerequisites:** The file `data/addressbook.json` contains multiple athletes.
    2. **Test case:** `list`

   **Expected:**
   All athletes are shown in the athlete list panel, with their information correctly matching the contents of the JSON file.
   If using the sample `addressbook.json`, the following should be displayed:
...
1. **Adding a valid athlete while all athletes are being shown**

    1. **Prerequisites:** List all athletes using the `list` command.
    2. **Test case:**

       `add n/Muhammad Irfan a/24 p/92345678 e/irfan24@example.com ad/45 Tampines Street 81, #10-22 d/30/06/2023 ec/Father 93456789 t/marathon av/Sat`

   **Expected:**
   The following success message is shown in the result display:
   ...
   **Expected:**
   The athlete's information is shown in the last row of the athlete list panel.

   ![Add athlete result in person list panel](images/add_athlete_result_in_person_list_panel.png)

   **Expected:**
   The athlete's information is also appended to the end of `addressbook.json`.

2. **Adding a valid athlete while only a filtered subset of athletes is being shown**

    1. **Prerequisites:** `addressbook.json` contains multiple athletes.
    2. **Prerequisites:** Use the `find` command to display only a filtered list of athletes such that fewer athletes are shown than stored.
       Example: `find n/Roy`
    3. **Test case:**
   ...
   **Expected:**
   The result is the same as adding a valid athlete when all athletes are shown.

3. **Adding an invalid athlete while all athletes are being shown**

    1. **Prerequisites:** List all athletes using the `list` command before each test.

    2. **Test case:**
       `add n/Chloe Ong n/Chris Ryan a/17 p/96543210 e/chloe.ong@example.com ad/9 Pasir Ris Drive 6, #07-44 d/08/04/2026 t/relay t/school av/Fri`

   **Expected:**
   `Multiple values specified for the following single-valued field(s): n/`

    3. **Test case:**
       `add n/Chloe Ong a/17 p/965432100 e/chloe.ong@example.com ad/9 Pasir Ris Drive 6, #07-44 d/08/04/2026 t/relay t/school av/Fri`

   **Expected:**
   `Phone number must be exactly 8 digits and start with 8 or 9 (e.g. 91234567).`

    4. **Test case:**
       `add n/Chloe Ong a/17 p/96543210 e/chloe.ong@ ad/9 Pasir Ris Drive 6, #07-44 d/08/04/2026 t/relay t/school av/Fri`
...
1. **Editing an athlete at a valid index when all athletes are being shown**

    1. **Prerequisites:** List all athletes using the `list` command.
    2. **Test case:**

       `edit 3 n/Lucas Tan a/21 p/91234567 e/lucas.tan@example.com ad/31 Serangoon North Ave 4 ec/Father 92345678 d/15/02/2024 t/teamB`
...
2. **Editing an athlete at a valid index when only one athlete is being shown**

    1. **Prerequisites:** Run `find n/Lucas` to display one athlete.
    2. **Test case:**
   ...
   **Expected:**
   The updated information is reflected in the athlete list panel.

   ![Edited Lucas result](images/edited_lucas_result.png)

3. **Editing an athlete with invalid format**

    1. **Prerequisites:** Run `list`.
    2. **Test case:**

       `edit 1 n/Lucas Tan a/21 p/9123 e/lucas.tan@example.com ad/31 Serangoon North Ave 4 ec/Father 92345678 d/15/02/2024 t/teamB`

   **Expected:**
   `Phone number must be exactly 8 digits and start with 8 or 9 (e.g. 91234567).`

4. **Editing an athlete with an invalid index**
   ...
   **Expected:**
   ...
   edit: Edits the details of the person identified by the index number...
   ```
...
1. **Deleting an athlete while all athletes are being shown**
   ...
   **Expected:**
   The first athlete is deleted and shown in the result display.

   Example:
   `Deleted athlete profile: Irfan Ibrahim; Age: 20; Phone: 92492021; ...`

2. **Deleting an athlete while only one athlete is being shown**

    1. **Prerequisites:** Run `find n/Lucas`.
    2. **Test case:** `del 1`

   **Expected:**
   Athlete is deleted and no longer appears after running `list`.

3. **Deleting an athlete with an invalid index**
   ...
   **Expected:**
   ...
   ```text
   Invalid command format!
   del: Deletes the athlete profile identified by the index number...
   ```
...
**Note:**
Find athlete commands are meant to pick out a smaller subset of athletes from a larger group.

Thus, all find athlete tests have the following common prerequisite:

**Prerequisite:**
Multiple contacts are present in the address book. For convenience, use the sample `addressbook.json` above.

All find athlete tests also have the following common expected result from running the test command:

**Expected:**
The result display should show the number of persons listed.
...
1. **Find athlete by name**
   ...
   **Expected:**
   Only athletes whose name contains `Irfan`, or any other chosen keyword, should appear in the person list panel.

2. **Find athlete by phone number**

    1. **Find athlete by full phone number**

       **Test case:**
       `find p/92492021` if using the sample `addressbook.json`, or `find p/x`, where `x` is any full and valid phone number.

       **Expected:**
       Only one athlete with the chosen phone number should appear in the person list panel.

    2. **Find athlete by partial phone number**

       **Test case:**
       `find p/92` if using the sample `addressbook.json`, or `find p/x`, where `x` is any partial phone number.

       **Expected:**
       All athletes whose phone number contains `x` as a substring, and only these athletes, should appear in the person list panel.

3. **Find athlete by a tag**

    1. **Finding athlete using an all-lowercase tag in the command**

       **Test case:**
       `find t/classmates` if using the sample `addressbook.json`, or `find t/TAG`, where `TAG` can be any tag keyword.

       **Expected:**
       All athletes that have the specified tag, and no other athletes besides these, should appear in the contact list.
...
4. **Find athlete by combination of tags**

    1. **Test case:**
       `find t/classmates t/colleagues` when using the sample `addressbook.json`, or `find t/TAG1 t/TAG2 ...`

   **Expected:**
   All athletes that have at least one of the specified tags, and no other athletes besides these, should appear in the contact list.

5. **Find athlete by combination of tags and full phone number**

    1. **Test case:** `find t/classmates p/924`

   **Expected:**
   All athletes that have at least one of the specified tags and the partial phone number, and no other athletes besides these, should appear in the contact list.

6. **Find a non-existent athlete by a non-existent name**

    1. **Test case:**
       `find n/Jerry` if using the sample `addressbook.json`, or `find n/x`, where `x` is a word that does not appear in any contact name.

   **Expected:**
   No athlete should be displayed in the person list panel.

7. **Find an athlete with invalid command format**

    1. **Test case:** `find q/Irfan`

   **Expected:**
   The following error message should be displayed in the result display:
...
2. At least one athlete is on display on the person list panel, for example by running the `list` command.
...
2. **Delete a valid run record index for an invalid athlete index**
...
3. **Delete an invalid run record index for a valid athlete index**
    ...
    2. **Use a run record index that is greater than the number of run records for that athlete**
...
1. **View the profile of an athlete at a valid index when all the athletes are on the person list panel**
    ...
    1. **Prerequisites:** View the list of all athletes by entering the `list` command.
...
2. **View the profile of an athlete at a valid index when only one athlete is on the person list panel**

    1. **Prerequisites:** Run the command `p/x`, where `x` is a valid full phone number of any athlete. This will output only one athlete in the person list panel as no duplicate phone numbers are allowed.

    2. **Test case:** `view 1`

       **Expected:** The full profile of the athlete should be shown in the result display. This includes their run timing records.

3. **View the profile of an athlete at an invalid index**
    ...
    2. **Choose an index greater than the number of athletes in the person list panel**
...
2. Each distance has to contain several athletes that run those distances, for example, 3 different athletes have a 2.4km time recorded, 5 different athletes have a 10km time recorded, etc.
...
1. **Sorting by name**
    ...
    1. **Sort ascending**
       ...
       **Expected:** The list of athletes is sorted by their names in alphabetical order from top to bottom, for example, names starting with `A` are at the top and names starting with `Z` are at the bottom.
    ...
    2. **Sort descending**
       ...
       **Expected:** The list of athletes is sorted by their names in reverse alphabetical order from top to bottom, for example, names starting with `Z` are at the top and names starting with `A` are at the bottom.
...
3. **Sorting using invalid distance field**

    1. **Test case:**

       **Expected:**

4. **Sorting using invalid order field**
...
1. **Clearing when there are no athletes in `addressbook.json`**
    ...
    2. **Test case:** `clear`
       ...
       **Expected:** Entering the `list` command will display an empty person list panel.

       **Expected:** After closing the window or running the `exit` command, the `addressbook.json` will be an empty list.
...
1. **Entering the help command**
    ...
    1. **Test case:** `help`
       ...
       **Expected:** There should be no changes to the person list panel.

---

### Exit

1. **Exiting the programme after no changes are made to the address book**

    1. **Test case:** `exit`

       **Expected:** The programme window closes, and all modifications done to the address book should be reflected in `addressbook.json`.

2. **Exiting the programme after adding a new athlete to the address book with run times**

    1. **Prerequisites:** Run the following commands in order:
